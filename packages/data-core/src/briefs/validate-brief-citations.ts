...
const NUMERIC_TOKEN = /(?<![\p{L}\p{N}$ۣ+\-])(?<sign>[+-]?)(?<currency>[$ۣ])?(?<value>\d+(?:[.,]\d+)*)(?<exponent>e[+-]?\d+)?(?<suffix>%|[kmb])?(?![\p{L}\p{N}])/giu;
...
function numericTokens(value: string): string[] {
  return [...value.matchAll(NUMERIC_TOKEN)].map((match) => {
    ...
    const magnitude = suffix && suffix in MAGNITUDE
      ...
      : 1;
    const normalizedValue = Number(`${sign}${numericValue.replace(/,/g, "")}${exponent}`) * magnitude;
