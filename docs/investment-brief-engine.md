...
`--thesis-file` also accepts the earlier bare-thesis shape for compatibility. A bare thesis is validated but contributes no parse metadata; the engine never fabricates a missing provider record. New parse-only runs should always use the proposal envelope so parse provenance survives confirmation.
...
The run mechanically retained 54 generation records: one thesis-parse and 50 claim-extraction responses from `gpt-5.6-luna`, plus three brief responses from `gpt-5.6-sol`. All 54 have response IDs, usage, requested/actual model, prompt version, trusted time, and the appropriate task/company/thesis context; aggregate usage is 60,228 input, 9,133 output, and 69,361 total tokens. The parse record is identical to the safe record carried through the reviewed proposal envelope. Stage evidence remains unsupported throughout the catalog.
