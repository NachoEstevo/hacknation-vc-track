...
The final accepted run generated at `2026-07-19T01:20:12.171Z` evaluated and ranked all 50 companies. It requested three briefs, generated two citation-valid briefs, and retained one sanitized draft failure, so the result is honestly `partial`. The B2B-software thesis remains one weight-preserving executable composite: 2 companies conflict, 7 are partial, and 41 are missing; none has cited support for both sides. Visible execution is deterministic from product, pricing, changelog, or GitHub signals. Ranking uses descending raw Thesis Fit, Evidence Coverage, Product/Execution score, then stable company ID. The top three are Icon, Steal These Thoughts!, and Julian Jewel's AI Bot. The artifact also retains 53 safe provider metadata records with actual/requested models, response IDs, token usage, prompt version, trusted generation times, and task/company/thesis context. Public enrichment with both a resolved-domain and profile-name mismatch is excluded before evaluation; Clay remains internal and unpublished.
